 ...
 * 
 * A common use case for configs is to have defaults (application.conf) and
 * overrides (application.{env}.conf) per environment. Typesafe Config is great
 * for this use case. Here is a mini wrapper for building Typesafe Config
 * objects.
 * 
 * From this code you have two global configurations set up: 1. System - This
 * has tons of useful JVM properties as well as any -D params you pass to the
 * JVM. 2. Properties - All of application.{env}.conf merged application.conf -
 * All general configs and defaults. application.local.conf - Overrides for
 * local env. .gitignore this file so each developer can have their own version.
 * application.prod.conf - Prod config, to use it just pass -Denv=prod to the
 * JVM.
 * 
 */
// {{start:config}}
public class Configs {
	...
	/*
	 * I am letting the typesafe configs classes bleed out on purpose here.
	 * 
	 * We could abstract out and delegate but its not worth it. I am gambling on the
	 * fact that I will not switch out the config library.
	 */

	// This config has all of the JVM system properties including any custom -D
	// properties
	...
	public static void initProperties(Config config) {
		...
		if (!success) {
			throw new RuntimeException(
					"propertiesRef Config has already been initialized. This should only be called once.");
		}
	}
	...
	public static <T> T getOrDefault(Config config, String path, BiFunction<Config, String, T> extractor,
			T defaultValue) {
	...
	public static Map<String, Object> asMap(Config config) {
		return Seq.seq(config.entrySet()).toMap(e -> e.getKey(), e -> e.getValue().unwrapped());
	}
